...
11. **Merge** the changes from the `new-feature` branch into the main branch and **delete** the `new-feature` branch.

:::important
Make sure to use the `--noedit` flag when merging, or do not change the default commit message. This will help the autograder identify the commit message.
:::
...
6. Checkout to main on your local repository and **pull** the changes from GitHub. You should see the changes you made in `conflict1` in your local repository.
...
8. Decide if you want to keep your changes from `conflict1` or from `conflict2`. Delete the conflict markers (the `<<<<<<<`, `=======`, and `>>>>>>>` lines) and complete there merge. Merge the pull request and pull the changes back into your local main branch.
...
Then make sure that your initial `echo >> "Hello Bash!"` command is in this file. If not, try deleting the contents of the file and running the command again but with more lines.
