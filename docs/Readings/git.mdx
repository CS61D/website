...
Once you have [created a git repository](#git-init---initialize-a-git-repository) (repo), git does not automatically start saving (tracking) changes. The edits which are saved to your local device but not to git are called your **working tree**. To save these changes to git, we create what is called a commit.
